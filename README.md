...
> ⚠️ **Please change your password immediately after first login** — go to Settings → Login Password. The default password is public knowledge.
<br><br>
<br><br>
  <p><i>("If you encounter bugs or installation problems, please submit an issue first.")</i></p>
<br><br>
<br><br>
...
> ⚠️ **首次登录后请立即修改密码** —— 进入设置 → 登录密码。默认密码是公开信息，不改等于裸奔。
<br><br>
<br><br>
<p><i>(如果遇到 Bug 或安装问题，请优先提交 Issue。)</i></p>
<br><br>
<br><br>
...
**4. 鸡公伴侣**
VPS的 CPU 跑满了或内存快爆了，后台会实时监控，并在 UI 页面上弹气泡吐槽，点一下VPS，会随机触发不同吐槽。例如（“老板，内存快吃光啦！”）。我本想做成一只电子鸡，但是工程量太大了，很多128MB的VPS可能吃不消就放弃了。
...
**6. 羊毛鸡保活**
很多「永久免费」的 VPS（如甲骨文等）都有回收机制：如果长期闲置（CPU 或内存占用过低），系统会判定资源浪费强制回收。鸡公头面板内置了智能的「心跳保活」功能：无需手动刷流量，它会以极低的资源消耗（<20MB RAM）维持那条「生存红线」，确保你的免费实例真正做到「永久免费」。
<br><br>
<br><br>
<p><b>哪些VPS可以安装？</b></p>
...
<details>
<summary><b>🇯🇵 日本語版 (クリックで展開/折りたたみ)</b></summary>
<br>

<div align="center">
  <h2>popodash (ミニマリスト VPS パネル)</h2>
  <p><b>ホコリをかぶったポテトVPSのためのサイバートイ</b></p>
  <p><i>「128MBのポテトサーバーにだって、カッコいいダッシュボードを。」</i></p>
</div>
...
デフォルトポートは `2077`（Cyberpunk 2077へのオマージュ）。変更も可能です。

インストール完了後、スクリプトがアクセスURL（IPv4・IPv6対応）を直接表示します。

- **デフォルトユーザー名**: `admin`
- **デフォルトパスワード**: `123456`

> ⚠️ **初回ログイン後、必ずパスワードを変更してください** —— 設定 → ログインパスワード。デフォルトパスワードは公開情報です。
<br><br>

<p><b>popodashの機能</b></p>

**1. エッグレーダー (ハードウェア情報)**
ホーム画面の卵をタップすると、主要メディアプラットフォームへのリアルタイム遅延を測定。GoogleやNetflixに繋がるか即座に確認できます。

**2. グローバルノード遅延テスト**
50か国以上のノードを内蔵。中央の「イースターエッグ」をクリックすると、世界中のデータセンターへの遅延をレーダーアニメーションで表示します。実用性ゼロ、視覚的満足感100%。

**3. 去勢ターミナル (Nerfed Terminal)**
読み取り専用サンドボックスで動くコマンドラインウィンドウ。`ping`、`top`、`neofetch`、果ては `rm -rf /` も入力できますが、サーバー上のファイルは絶対に削除されません。安全に「玄人感」を楽しめます。

**4. ロースターコンパニオン**
CPUが高負荷・メモリが逼迫すると、リアルタイムで監視してUIにチャットバブルで文句を言います。VPSのアイコンをタップするとランダムな愚痴が飛び出します。

**5. Telegram インテル (定期レポート)**
Telegramボットを連携して時刻を設定するだけ。毎日、秘書のようにトラフィック消耗・SSHログイン監査をまとめてスマホに送信。即時レポートの送信も可能。

**6. 無料VPS 延命機能**
Oracle Always FreeなどのVPSはアイドル状態が続くと回収されることがあります。popodashは超低リソース（RAM 20MB未満）でハートビートを維持し、「永久無料」インスタンスを本当に永久無料のままにします。
<br><br>

<p><b>対応VPS環境</b></p>

- **フリービー神機**: Oracle Always Free、Serv00、Euserv (純IPv6)
- **コスパ戦士**: Cloudcone、RackNerd、BandwagonHostなどの低スペックVPS
- **極限環境**: NAT、純IPv6、128MB RAMのミニマルVPS

SSHに接続してスクリプトをコピー＆ペーストするだけ。環境を自動検出し、足りないものを自動補完します。
<br><br>

<p><b>popodash。大きな依存関係は不要、RAM使用量20MB未満。</b></p>
深夜の安らぎの場所——サーバー状態を確認し、レーダーテストを走らせ、サンドボックス端末でインジケーターが点滅するのを眺める場所。

小さなポテトサーバーにも、美しい魂を。楽しんでください！

</details>

<br><br>
<div align="center">
  <a href="https://popodash.uepopo.com/"><img src="https://img.shields.io/badge/Website-popodash-green?style=flat-square&logo=googlechrome" alt="Website"></a>
  &nbsp;
<a href="https://mail.google.com/mail/?view=cm&fs=1&to=uepopo@autistici.org" target="_blank">
  <img src="https://img.shields.io/badge/Email-UEPOPO-gray?style=flat-square&logo=gmail" alt="Email">
</a>
  &nbsp;
